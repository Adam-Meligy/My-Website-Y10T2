function searchFunction() {
    ...
    // this declares variables that hold refrence to values and html elements, they will be defined later.
    ...
    for (i = 0; i < li.length; i++) {
        ...
        if (txtValue.toUpperCase().indexOf(filter) > -1) {
            ...
            // .indexOf(filter) takes the "txtValue" string and determins if a specific substring, filter, occurs within that string. this is the actual search function. Since "txtValue = a.textcontent", there are multiple <a> tags it will look through. it will look through each one, one by one. if it finds a "txtValue" value that is contained in the "filter" value, then it will return its index position in the word("txtValue") itself. as mentioned earler, 0 is the beginning of the list, increases by 1 as you go down). for example, the filer value is e, and it looks through the txtValue "Elder Hu" first, it will return 0, as e is first found in the first letter in the word. it will then do the same for the next value of "txtValue", until it has returned values for all of them. if the "filter" value is not found in the "txtValue" string, then the .indexOf function will return -1.
            ...
            a.style.display = "";
            // sets the display (css code) of the "a" variables (the <a> links) to its css display value, and removes any inline display styles that have been set to that element through javascript code - it just goes to its display style in css.
        } else {
            // the if statement runs the code if the statment is true. when adding "else", it will run a different line of code if the if statment is false.
            a.style.display = "none";
            // this will be how the searchbar hides the links that dont match what was typed in the search bar. if the statement is false, meaning the "filter" value wasn't found in the "txtValue" elements, then this code will run
            // sets the display of the "a" variables (the <a> links) to "none". setting an element's display to none means it doesnt show up on the actual website. this is how words that dont have what was typed in the searchbar are hidden.
        }
    }
}